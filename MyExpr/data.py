...
class Data(object):
    ...
    def non_iid_pathological(self):
        ...
        for client_id in range(args.client_num_in_total):
            ...
            self.train_set.append(client_train_dataset[client_id])
            self.validation_set.append(client_validation_dataset[client_id])
            self.test_set.append(client_test_dataset[client_id])
    ...
    def non_iid_pathological2(self):
        ...
        for i in range(len(dic_test_data)):
            ...
            self.test_non_iid.append(
                DataLoader(class_test_dataset[i], batch_size=args.batch_size, shuffle=True,
                           num_workers=args.num_workers))
            self.non_iid_test_set[i] = class_test_dataset[i]
    ...
    def pathological2_helper(self, dataset):
        ...
        client_id = 0
        self.dist_client_dict = {dist_id : [] for dist_id in range(args.num_distributions)}
        self.client_dist_dict = {}
        ...
        for dist_id in range(args.num_distributions):
            for i in range(args.num_clients_per_dist):
                ...
                self.dist_client_dict[dist_id].append(client_id)
                self.client_dist_dict[client_id] = dist_id
                client_id += 1
    ...
    def non_iid_latent(self):
        ...
        # clustering 这种distribution划分方法有问题
    ...
    def iid(self):
        ...
        dict_users_test = {}
        for i in range(args.client_num_in_total):
            off, l = cumsum_test[i], num_test[i]
            dict_users_test[i] = idx_test[off - l: off]
        splited_testset = [Subset(testset, dict_users_test[i]) for i in range(args.client_num_in_total)]

        dict_users_train = {}
        for i in range(args.client_num_in_total):
            off, l = cumsum_train[i], num_train[i]
            dict_users_train[i] = idx_train[off - l : off]
        self.train_idx_dict = dict_users_train

        train_dataset, validation_dataset = self.validation_partition(dict_users_train, trainset, args)
        ...
        for c_id in range(args.client_num_in_total):
            ...
            print(f"client {c_id} test (labels, counts) {[(i, count) for i, count in enumerate(counts)]}")

        for i in range(args.client_num_in_total):
            self.train_loader.append(DataLoader(train_dataset[i], batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers))
            self.train_set.append(train_dataset[i])
            self.validation_loader.append(DataLoader(validation_dataset[i], batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers))
            self.validation_set.append(validation_dataset[i])
            self.test_loader.append(DataLoader(splited_testset[i], batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers))
            self.test_set.append(splited_testset[i])
    ...
    def init_clients_data(self, distributions):
        ...
        for ix, dist in enumerate(distributions):
            ...
            self.test_non_iid[ix] = DataLoader(non_iid_test_set, batch_size=args.batch_size, shuffle=True,
                                               num_workers=args.num_workers)
            self.non_iid_test_set[ix] = non_iid_test_set
        ...
        for ix in range(len(distributions)):
            ...
            for i in range(args.num_clients_per_dist):
                ...
                self.train_set.append(client_data[3])
                self.validation_set.append(client_data[4])
                self.test_set.append(client_data[5])
