...
def train(net, dataloader, optimizer, criterion, epoch, args):
    ...
    print(
        f'Epoch: {epoch} | Train accuracy: {100 * correct / total:<.2f}% | Train loss: {running_loss / len(dataloader):<.4f}')
...
def evaluate(net, dataloader, criterion, epoch, args):
    ...
    with torch.no_grad():
        for i, data in enumerate(tqdm(dataloader, desc='Evaluating')):
            ...
            correct += (predicted == labels).sum().item()
        print(
            f'Epoch: {epoch} | Eval accuracy: {100 * correct / total:<.2f}% | Eval loss: {running_loss / len(dataloader):<.4f}')
    return running_loss, correct / total
